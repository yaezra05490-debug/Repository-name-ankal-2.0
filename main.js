  ...
  return new Promise((resolve, reject) => {
    const server = http.createServer(async (req, res) => {
      try {
        ...
        const redirectUri = `http://127.0.0.1:${server.address().port}/oauth/callback`; const body = new URLSearchParams({ client_id: config.googleDesktopClientId, code, code_verifier: verifier, grant_type: "authorization_code", redirect_uri: redirectUri });
        const tokenResponse = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body }); const tokens = await tokenResponse.json(); if (!tokenResponse.ok || !tokens.id_token) throw new Error("TOKEN_EXCHANGE_FAILED");
      ...
      } catch (error) { res.writeHead(400, { "Content-Type": "text/plain; charset=utf-8" }); res.end("הכניסה לא הושלמה. אפשר לסגור את החלון."); reject(error); server.close(); }
    });
